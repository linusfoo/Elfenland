...
import easygui
from tkinter import Event, Tk, Variable, simpledialog, Label, StringVar, Button
import time
import socket
...
def main(username,variant,numPlayers,host,game_id,saveID):
        ...
        for path in game.AllPaths.paths:
            if path.transportCounter is not None:
                ...
                if path.has_obstacle:
                    screen.blit(OBS_MAP, (path.tcx + 12, path.tcy + 12)) #Testo
    ...
    pygame.mixer.music.load(sourceFileDir + '/assets/background.mp3')
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_p:
                    print("pause music")
                    pygame.music.mixer.pause()

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_u:
                    print("unpause music")
                    pygame.music.mixer.unpause()
    ...
    while run:
        ...
        if (m!= None):
            ...
            if GAME.currentPhase == 3:
                p.message_box = str("Choose a Transport Counter to draw")
            if GAME.currentPhase == 6:
                p.message_box = str("Choose a Transport Counter to remove")
